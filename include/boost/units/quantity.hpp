...
class quantity
{
    ...
    public:
        ...
        /// implicit conversion between value types is allowed if allowed for
        ...
        /// implicit conversion between value types is not allowed if not
        /// allowed for value types themselves
        template<class YY>
        ...
        /// implicit assignment between value types is allowed if allowed for
        /// value types themselves
        template<class YY>
        ...
        /// explicit conversion between different unit systems is allowed if
        ...
        /// implicit conversion between different unit systems is allowed if
        /// each fundamental dimension is implicitly convertible
        template<class Unit2,class YY>
        ...
        /// without SFINAE we can't distinguish between explicit and implicit
        /// conversions so the conversion is always explicit
        template<class Unit2,class YY> 
        ...
        /// implicit assignment between different unit systems is allowed if
        /// each fundamental dimension is implicitly convertible
        template<class Unit2,class YY>
        ...
        /// can add a quantity of the same type if
        /// add_typeof_helper<value_type,value_type>::type is convertible to
        /// value_type
        template<class Unit2, class YY>
        BOOST_CONSTEXPR this_type& operator+=(const quantity<Unit2, YY>& source)
        {
            BOOST_STATIC_ASSERT((boost::is_same<typename add_typeof_helper<Unit, Unit2>::type,
                                 Unit>::value));
            val_ += source.value();
            return *this;
        }

        /// can subtract a quantity of the same type if
        /// subtract_typeof_helper<value_type,value_type>::type is convertible
        /// to value_type
        template<class Unit2, class YY>
        BOOST_CONSTEXPR this_type& operator-=(const quantity<Unit2, YY>& source)
        {
            BOOST_STATIC_ASSERT((boost::is_same<typename subtract_typeof_helper<Unit, Unit2>::type,
                                 Unit>::value));
            val_ -= source.value();
            return *this;
        }
        ...
        BOOST_CONSTEXPR this_type& operator*=(const quantity<Unit2, YY>& source)
        {
            BOOST_STATIC_ASSERT((boost::is_same<typename multiply_typeof_helper<Unit, Unit2>::type,
                                 Unit>::value));
            val_ *= source.value();
            return *this;
        }
        ...
        BOOST_CONSTEXPR this_type& operator/=(const quantity<Unit2, YY>& source)
        {
            BOOST_STATIC_ASSERT((boost::is_same<typename divide_typeof_helper<Unit, Unit2>::type,
                                 Unit>::value));
            val_ /= source.value();
            return *this;
        }

        /// can multiply a quantity by a scalar value_type if
        /// multiply_typeof_helper<value_type,value_type>::type is convertible
        /// to value_type
        BOOST_CONSTEXPR this_type& operator*=(const value_type& source)
        {
          val_ *= source;
          return *this;
        }
        /// can divide a quantity by a scalar value_type if
        /// divide_typeof_helper<value_type,value_type>::type is convertible to
        /// value_type
        BOOST_CONSTEXPR this_type& operator/=(const value_type& source)
        {
          val_ /= source;
        ...
        BOOST_CONSTEXPR static this_type from_value(const value_type& val)
...
class quantity<BOOST_UNITS_DIMENSIONLESS_UNIT(System),Y>
{
    public:
        ...
        BOOST_CONSTEXPR explicit quantity(const quantity<unit<dimensionless_type, System2>,Y2>& source,
            ...
            val_(conversion_helper<quantity<unit<dimensionless_type, System2>,Y2>,
                 this_type>::convert(source).value()) 
        {
            BOOST_UNITS_CHECK_LAYOUT_COMPATIBILITY(this_type, Y);
        }
        ...
        /// multiply_typeof_helper<value_type,value_type>::type is convertible
        /// to value_type
        BOOST_CONSTEXPR this_type& operator*=(const value_type& val)
...
struct quantity_cast_helper<Y,quantity<Unit,X> >
{
    ...
    BOOST_CONSTEXPR type operator()(quantity<Unit,X>& source)
...
struct quantity_cast_helper<Y,const quantity<Unit,X> >
{
    ...
    BOOST_CONSTEXPR type operator()(const quantity<Unit,X>& source)
    { return source.value(); }
};
...
BOOST_CONSTEXPR inline void swap(quantity<Unit,Y>& lhs, quantity<Unit,Y>& rhs)
{
    using std::swap;
